...
打开 `platform.deepseek.com/usage`，官方页面默认只展示**本月累计**数据。如果你想看「今天花了多少钱」「今天请求了多少次」「缓存命中率是多少」，需要自己点图表柱子、点日期、手动算缓存命中率。
...
2. 点击脚本管理器的「添加新脚本」，将本仓库中 `deepseek-usage-injector.user.js` 的**全部内容**粘贴进去，保存。

3. 脚本会自动生效。你什么都不用配置。

## 使用方法

安装后直接打开 [DeepSeek Platform 用量页面](https://platform.deepseek.com/usage)，你会发现多了以下内容：
